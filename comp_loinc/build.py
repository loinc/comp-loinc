...
TODO's
  1. Named args: Change args from positional to named, e.g.: https://typer.tiangolo.com/tutorial/parameter-types/number/
  2. Path resolution / graceful exiting: If path of a file/dir doesn't exist, should exit gracefully early on. I left
  exists=False for each `typer.Option` as a reminder. I would use `exists=True`, but `typer` has a relative path bug.
  3. Consider changing/adding docstring param descriptions to `typer.Option(help=<description>)`.
"""
import os
import subprocess
