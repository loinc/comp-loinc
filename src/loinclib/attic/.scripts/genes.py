...
release: LoincGraph = LoincGraph(
    release_path=Path("data/loinc_release/2.67"),
    trees_path=Path("data/loinc_release/2.67/trees/2023-09-26"),
    loinc_version="2.6.7",
)
...
mapping_frame: DataFrame = (
    release.read_AccessoryFiles_PartFile_PartRelatedCodeMapping_csv()
)
    ...
    if part_type == "GENE" and part_number not in gene_parts:
        if part_number not in gene_parts:
            series: Series = Series(
                data=[part_number, part_name, part_type, "", "", "", "", "", "", ""],
                index=index,
            )
