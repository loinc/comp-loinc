...
release = LoincGraph(
    project_path / "data" / "loinc_release" / "extracted",
    "2.7.4",
    project_path / "data" / "loinc_trees" / "2023-06-14",
)
...
for (
    tpl
) in release.read_AccessoryFiles_PartFile_LoincPartLink_Primary_csv().itertuples():
    (
        row_number,
        loinc_number,
        long_common_name,
    ...
    rel = f"{link_type_name}_{part_type_name}"
