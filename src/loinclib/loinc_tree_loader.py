...
from loinclib import LoinclibGraph, GeneralEdgeType
...
class LoincTreeLoader:
  ...
  def load_nlp_tree(
    ...
    for tpl in df.itertuples():
      ...
      child_node.add_edge_single(
          type_=GeneralEdgeType.has_parent, to_node=parent_node
      )
  ...
  def _load_tree(self, source: LoincTreeSource, from_nlp_source=False, multi_axial=False, component_by_system=False):
    ...
    for parent_id, code, code_text in records_by_id.values():
      ...
      if parent_id:
        ...
        part_node.add_edge_single(
            type_=GeneralEdgeType.has_parent, to_node=parent_node, source=f"tree.{source}"
        )
