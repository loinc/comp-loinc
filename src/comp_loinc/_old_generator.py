...
class Generator:
    ...
    def __init__(
        ...
        self.schema_view = linkml_runtime.SchemaView(
            self.schema_dir / "comp_loinc.yaml", merge_imports=True
        )
    ...
    def add_part_info(self, part_codes: list[str] = None):
        ...
        for part_code in part_codes:
            ...
            part_name = self.loinc_release.get_first_node_property(
                PT.loinc_part_name, node_id
            )
            part.label = part_name
            part.part_name = part_name
            part.part_display_name = self.loinc_release.get_first_node_property(
                PT.loinc_part_display_name, node_id
            )
            part.part_type = self.loinc_release.get_first_node_property(
                PT.loinc_part_type, node_id
            )
    ...
    def generate_parts_list(self):
        part_node_ids = self.loinc_release.get_all_node_ids_for_node_type(
            NT.loinclib_part
        )
        ...
        for part_node_id in part_node_ids:
            ...
            part = self.parts.setdefault(
                code, LoincPart(id=_loincify(code), part_number=code)
            )

            part_name = self.loinc_release.get_first_node_property(
                PT.loinc_part_name, part_node_id
            )
            part.label = part_name
            part.part_name = part_name

            part.part_display_name = self.loinc_release.get_first_node_property(
                PT.loinc_part_display_name, part_node_id
            )
            part.part_type = self.loinc_release.get_first_node_property(
                PT.loinc_part_type, part_node_id
            )
    ...
    def generate_loincs_list(self):
        ...
        loinc_node_ids = self.loinc_release.get_all_node_ids_for_node_type(
            NT.loinc_code
        )
    ...
    def generate_loincs_primary_defs(self):
        ...
        for loinc_node_id in loinc_node_ids:
            ...
            # component
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_Primary_COMPONENT
            )
            if len(parts) > 1:
            ...
            # property
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_Primary_PROPERTY
            )
            if len(parts) > 1:
            ...
            # time
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_Primary_TIME
            )
            if len(parts) > 1:
            ...
            # system
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_Primary_SYSTEM
            )
            if len(parts) > 1:
            ...
            # scale
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_Primary_SCALE
            )
            if len(parts) > 1:
            ...
            # method
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_Primary_METHOD
            )
            if len(parts) > 1:
    ...
    def generate_loincs_supplementary_defs(self):
        ...
        for loinc_node_id in loinc_node_ids:
            ...
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_DetailedModel_COMPONENT_analyte
            )
            if len(parts) > 1:
                raise ValueError(
                    f"Loinc: {code} has multiple component analytes: {parts}"
                )
            for key, part_node_id in parts.items():
                part_code = NT.loinclib_part.identifier_of_nodeid(part_node_id)
            ...
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_DetailedModel_CHALLENGE_challenge
            )
            if len(parts) > 1:
                raise ValueError(
                    f"Loinc: {code} has multiple component challenges: {parts}"
                )
            for key, part_node_id in parts.items():
                part_code = NT.loinclib_part.identifier_of_nodeid(part_node_id)
            ...
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_DetailedModel_ADJUSTMENT_adjustment
            )
            if len(parts) > 1:
                raise ValueError(
                    f"Loinc: {code} has multiple component adjustments: {parts}"
                )
            for key, part_node_id in parts.items():
                part_code = NT.loinclib_part.identifier_of_nodeid(part_node_id)
            ...
            # 1.4 component count
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_DetailedModel_COUNT_count
            )
            if len(parts) > 1:
                raise ValueError(
                    f"Loinc: {code} has multiple component counts: {parts}"
                )
            for key, part_node_id in parts.items():
                part_code = NT.loinclib_part.identifier_of_nodeid(part_node_id)
            ...
            # 3.1 time core
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_DetailedModel_TIME_time_core
            )
            if len(parts) > 1:
            ...
            # 3.2 time modifier
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_DetailedModel_TIME_MODIFIER_time_modifier
            )
            if len(parts) > 1:
                raise ValueError(
                    f"Loinc: {code} has has multiple time modifiers: {parts}"
                )
            for key, part_node_id in parts.items():
                part_code = NT.loinclib_part.identifier_of_nodeid(part_node_id)
            ...
            # 4.1 system core
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_DetailedModel_SYSTEM_system_core
            )
            if len(parts) > 1:
                raise ValueError(
                    f"Loinc: {code} has has multiple system cores: {parts}"
                )
            for key, part_node_id in parts.items():
                part_code = NT.loinclib_part.identifier_of_nodeid(part_node_id)
            ...
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_DetailedModel_SUPER_SYSTEM_super_system
            )
            if len(parts) > 1:
                raise ValueError(
                    f"Loinc: {code} has has multiple system super systems: {parts}"
                )
            for key, part_node_id in parts.items():
                part_code = NT.loinclib_part.identifier_of_nodeid(part_node_id)
            ...
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_SemanticEnhancement_GENE_analyte_gene
            )
            if len(parts) > 1:
                raise ValueError(
                    f"Loinc: {code} has has multiple semantic genes: {parts}"
                )
            for key, part_node_id in parts.items():
                part_code = NT.loinclib_part.identifier_of_nodeid(part_node_id)
            ...
            # syntax analyte core
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_SyntaxEnhancement_COMPONENT_analyte_core
            )
            if len(parts) > 1:
                raise ValueError(
                    f"Loinc: {code} has has multiple syntax analyte core: {parts}"
                )
            for key, part_node_id in parts.items():
                part_code = NT.loinclib_part.identifier_of_nodeid(part_node_id)
            ...
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_SyntaxEnhancement_SUFFIX_analyte_suffix
            )
            if len(parts) > 1:
                raise ValueError(
                    f"Loinc: {code} has has multiple syntax analyte core: {parts}"
            ...
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_SyntaxEnhancement_DIVISOR_analyte_divisor
            )
            if len(parts) > 1:
                raise ValueError(
                    f"Loinc: {code} has has multiple syntax analyte divisor: {parts}"
                )
            for key, part_node_id in parts.items():
                part_code = NT.loinclib_part.identifier_of_nodeid(part_node_id)
            ...
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_SyntaxEnhancement_SUFFIX_analyte_divisor_suffix
            )
            if len(parts) > 1:
                raise ValueError(
                    f"Loinc: {code} has has multiple syntax analyte divisor suffixes: {parts}"
                )
            for key, part_node_id in parts.items():
                part_code = NT.loinclib_part.identifier_of_nodeid(part_node_id)
            ...
            parts = self.loinc_release.out_node_ids(
                loinc_node_id, ET.loinc_SyntaxEnhancement_NUMERATOR_analyte_numerator
            )
            if len(parts) > 1:
                raise ValueError(
                    f"Loinc: {code} has has multiple syntax analyte numerators: {parts}"
                )
            for key, part_node_id in parts.items():
                part_code = NT.loinclib_part.identifier_of_nodeid(part_node_id)
    ...
    def generate_parts_trees(self):
        ...
        for part_node_id in part_node_ids:
            ...
            # do subclass axioms
            parent_ids = self.loinc_release.out_node_ids(
                part_node_id, ET.loinc_LoincTree_has_parent_has_parent
            )
            if parent_ids:
                part.subClassOf = [
                    PartClassId(
                        _loincify(NT.loinclib_part.identifier_of_nodeid(parent_id))
                    )
                    for parent_id in parent_ids.values()
                ]
            else:
                part_parent = None
                child_node_ids = self.loinc_release.in_node_ids(
                    part_node_id, ET.loinc_LoincTree_has_parent_has_parent
                )
    ...
    def _parts_group_comp_equivalences_recurse(self, part_node_id, code_map):
        ...
        children_parts = self.loinc_release.in_node_ids(
            part_node_id, ET.loinc_LoincTree_has_parent_has_parent
        ).values()
        for child_part_id in children_parts:
            if NT.type_for_node_id(child_part_id) == NT.loinclib_part:
                child_def = self._parts_group_comp_equivalences_recurse(
                    child_part_id, code_map
                )
                child_def.subClassOf.append(
                    LoincTermNonIntersectionId(part_based_def.id)
                )
    ...
    def _generate_mapping(self, namespace: ll.NameSpaceEnum):
        ...
        for part_node_id in part_node_ids:
            part_identifier = ll.NodeType.loinclib_part.identifier_of_nodeid(
                part_node_id
            )
            # equivalence axioms
            equivalent_target_node_ids = self.loinc_release.out_node_ids(
                part_node_id, ET.loinc_Equivalence_equivalent_equivalent
            )
            for equivalent_target_node_id in [
                id
                for key, id in equivalent_target_node_ids.items()
                if target_node_type.is_type_of_nodeid(id)
            ]:
                loinc_part = part_map.setdefault(
                    part_identifier, LoincPart(NS.loinc.owl_url(part_identifier))
                )
                target: LoincEntity = LoincEntity(
                    namespace.owl_url(
                        target_node_type.identifier_of_nodeid(equivalent_target_node_id)
                    )
                )
                properties = self.loinc_release.get_node_properties(
                    equivalent_target_node_id
                )
    ...
    def _write_owl_outputs(self, /, *, output_name: str, timestamped=True):
        output_file = self.output_directory / f"{output_name}.owl"
        output_file_datetime = (
            self.output_directory
            / "datetime"
            / f"{output_name}_{self.__datetime_string}.owl"
        )
        ...
        document: funowl.ontology_document.OntologyDocument = (
            self._owl_dumper.to_ontology_document(
                element=list(self._outputs[output_name].values()),
                schema=self.schema_view.schema,
            )
        )
        document.ontology.iri = funowl.identifiers.IRI(
            f"https://loinc.org/{output_name}"
        )
    ...
    def _write_rdf_outputs(self, /, *, output_name: str, timestamped=True):
        output_file = self.output_directory / f"{output_name}.ttl"
        output_file_datetime = (
            self.output_directory
            / "datetime"
            / f"{output_name}_{self.__datetime_string}.ttl"
        )
    ...
    def _loinc_parent(self, properties, code_map: dict[str, LoincTerm]) -> LoincTerm:
        ...
        if class_types:
            ...
            class_type = code_map.setdefault(
                class_type_code,
                LoincTermIntersection(
                    id=_loincify(class_type_code),
                    label=f"class type {class_types[0]}",
                    subClassOf=[LoincTermId(code_map["__loincs"].id)],
                ),
            )
        ...
        if classes:
            ...
            for part in class_parts:
                if not class_part:
                    ...
                    class_part = code_map.setdefault(
                        url_suffix,
                        LoincTermIntersection(
