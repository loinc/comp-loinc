...
class BuilderCli:
    ...
    def set_current_module(
        ...
        name: t.Annotated[
            str,
            typer.Option("--name", "-n", help="Set the current module to this name."),
        ],
    ):
    ...
    def remove_module(
        self,
        module_name: t.Annotated[
            str,
            typer.Option(
                "--name", "-n", help="Removes the named module from the runtime."
            ),
        ],
    ):
...
class CompLoincCli:
    ...
    def callback(
        ...
        *,
        ...
        config_file: t.Annotated[
            t.Optional[Path],
            typer.Option(
                help='Configuration file name. Defaults to "comploinc_config.yaml"'
            ),
        ] = Path("comploinc_config.yaml"),
        output_dir: t.Annotated[
            t.Optional[Path],
            typer.Option(
                "--out-dir", "-o", help='The output folder name. Defaults to "output".'
            ),
        ] = "output",
        fast_run: t.Annotated[
            bool,
            typer.Option(
                help="Turns on a fast run feature which is useful for development.",
                hidden=True,
            ),
        ] = False,
    ):
        ...
        self.config = Configuration(
            home_path=self.work_dir, config_file=config_file.absolute()
        )
    ...
    def build(
        self,
        build_name: t.Annotated[
            Path,
            typer.Argument(
                help='The build name or a path to a build file. The "default" build '
                "will build all outputs."
            ),
        ] = "default",
    ):
