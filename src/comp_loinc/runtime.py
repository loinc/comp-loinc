...
class Runtime:
    ...
    def load_linkml_schema(
        self, file_name: str, as_name: str = None, reload: bool = False
    ) -> SchemaView:
        ...
        if current_view and not reload:
            ...
            raise ValueError(
                f"Schema view for name: {as_name} already loaded from file: {file}"
            )
        ...
        if schema_path.exists():
            ...
            raise ValueError(
                f"Schema file {schema_path} does not exist while trying to load as name: {as_name}"
            )
