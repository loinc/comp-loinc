...
class ChebiFhirIngest(Mappings):
    ...
    def get_fhir_chebi_mappings(self):
        r = requests.get(
            f"{self.fhir_endpoint}/ConceptMap/?url=http://loinc.org/cm/loinc-parts-to-chebi",
    ...
    def create_chebi_loinc_sssom(self):
        ...
        for part_map in part_mappings:
            ...
            sssom_obj["predicate_id"] = predicate_map[
                part_map["target"][0]["equivalence"]
            ]
        ...
        with open(
            f"{path_root}/data/output/sssom_mapping_files/loinc2chebi.tsv", "w"
        ) as lc:
    ...
    def sssom_chebi_to_owl(self):
        with open(f"{self.output}", "w") as l2co:
            convert_file(
                f"{path_root}/data/output/sssom_mapping_files/loinc2chebi.tsv",
                output=l2co,
                output_format="owl",
            )
