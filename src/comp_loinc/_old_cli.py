...
class CompLoincCli:
    ...
    def callback(
        ...
        *,
        work_dir: t.Annotated[
            t.Optional[Path], typer.Option(help="CompLOINC work directory.")
        ] = Path.cwd(),
        loinc_release: t.Annotated[
            t.Optional[Path],
            typer.Option(
                help=f"Path to a directory containing an unpacked LOINC release. Defaults to: ./{LOINC_RELEASE_DIR_NAME}"
            ),
        ] = None,
        loinc_trees: t.Annotated[
            t.Optional[Path],
            typer.Option(
                help=f"A directory name to load tree files from. Defaults to {LOINC_TREES_DIR_NAME}"
            ),
        ] = None,
        loinc_version: t.Annotated[
            t.Optional[str],
            typer.Option(
                help="The LOINC release version. It uses the directory name of the the loinc_release option by if it appears to be a release number."
            ),
        ] = None,
        out_dir: t.Annotated[
            t.Optional[Path], typer.Option(help="The CompLOINC output directory.")
        ] = None,
        log_level: t.Annotated[
            str, typer.Option(help="Logging level. Defaults to WARN.")
        ] = "WARN",
        ...
        self.loinc_release_path = CompLoincCli._find_loinc_release_path(
            loinc_release, self.work_dir
        )
        self.loinc_trees_path = CompLoincCli._find_trees_path(
            loinc_trees, self.work_dir
        )
        ...
        if re.match(r"[0-9]\.[0-9]+", loinc_version):
            ...
            raise ValueError(
                f"LOINC version {loinc_version} does not appear to be a valid version."
            )
    ...
    def _find_loinc_release_path(cls, loinc_dir: Path, work_dir: Path) -> Path:
        ...
        if not (loinc_release_path / "LoincTable").exists():
            raise ValueError(
                f"LOINC release path {loinc_release_path}/LoincTable is not found."
            )
    ...
    def _find_trees_path(cls, loinc_trees: Path, work_dir: Path) -> Path:
        ...
        if not (loinc_trees_path / "class.csv").exists():
            raise ValueError(
                f"LOINC tree file {loinc_trees_path / 'class.csv'} does not exist"
            )
