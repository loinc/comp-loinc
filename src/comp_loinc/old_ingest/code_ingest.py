...
class CodeIngest(object):
    ...
    def __init__(self, schema_path: str, code_file_path: str):
        print(
            f"Beginning Code Ingest at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
    ...
    def process_lpl_file(self):
        ...
        """
        return pd.read_csv(
            f"{self.code_file_path}/LoincPartLink_Primary.csv", sep=",", dtype=str
        )
    ...
    def concatentate_formal_name(self):
        ...
        self.code_dataframe["LoincFormalName"] = self.code_dataframe[cols].apply(
    ...
    def group_by_code(self):
        ...
        """
        group_map = {}
        groups = self.lpl_dataframe.groupby(["LoincNumber"])[
            ["PartNumber", "PartTypeName"]
        ]
    ...
    def generate_codes(self):
        ...
        for i, loinc_number in enumerate(included_codes):
            ...
            if loinc_number in self.group_map.keys():
                row = self.code_dataframe[
                    self.code_dataframe["LOINC_NUM"] == loinc_number
                ].iloc[0]
                params = {
    ...
    def write_output_to_file(self, output_path):
        ...
        print(
            f"Finished Code Ingest at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
