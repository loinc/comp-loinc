...
class PartOntology(object):
    ...
    def __init__(self, schema_path: str, part_file_directory_path: str):
        print(
            f"Beginning Part Ingest at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
    ...
    def load_part_files(self):
        ...
        for part_file in os.listdir(self.part_file_directory_path):
            part_file_dfs.append(
                pd.read_csv(f"{self.part_file_directory_path}/{part_file}", sep="\t")
            )
    ...
    def generate_ontology(self):
        ...
        :return:
        """
        part_groups = self.all_parts_df.groupby("ChildPartNumber")[
            ["ChildPart", "ChildPartTypeName", "ParentPartNumber"]
        ]
        for i, pg in enumerate(part_groups):
            counter(i + 1, len(part_groups))
            params = {"id": loincify(pg[0]), "part_number": pg[0]}
            ...
            parent_part_numbers = [
                loincify(x)
                for x in part_attributes["ParentPartNumber"].unique()
                if loincify(x) != params["id"]
            ]
    ...
    def write_to_output(self, output_path):
        ...
        with open(
            output_path, "w"
        ) as ccl_owl:  # ./data/output/owl_component_files/part_ontology.owl
            ccl_owl.write(
                self.od.dumps(
                    self.part_classes,
                    schema=self.sv.schema,
                )
            )
        print(
            "\n"
