...
class PartHierarchy(object):
    ...
    def generate_part_hierarchy(self):
        ...
        """
        node_id_to_part_id = {x.NODE_ID: x.FK_ID for x in self.hierarchy.itertuples()}
        ...
        self.hierarchy["parent_part_id"] = self.hierarchy["PARENT_ID"].apply(
            node_to_part
        )
    ...
    def generate_label_map(self):
        return {
            x.FK_ID: x.PART
            for x in self.hierarchy[["FK_ID", "PART"]].drop_duplicates().itertuples()
        }
...
class PartLookups(object):
    ...
    def combine_part_files_to_df(self):
        return pd.concat(
            [
                pd.read_csv(self.part_primary_file_path),
                pd.read_csv(self.part_supplementarty_file_path),
            ]
        )
    ...
    def generate_part_type_lookup(self):
        ...
        """
        return dict(
            zip(self.part_file["PartNumber"].tolist(), self.part_file["PartTypeName"])
        )
    ...
    def generate_part_name_lookup(self):
        ...
        """
        return dict(
            zip(self.part_file["PartNumber"].tolist(), self.part_file["PartName"])
        )
