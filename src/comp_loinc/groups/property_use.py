...
class Property:
  ...
  def get_descendants_count(self, seen: set):
    ...
    if self.descendants_count is None:
      i = sum([c.get_descendants_count(seen) for c in self.child_prop_use_by_key.values()])
  ...
  def abstract_to_more_than_count(self, count: int, seen: set) -> t.Dict[str, Property]:
    if self in seen:
      return {}
    seen.add(self)
    abstract = {}
