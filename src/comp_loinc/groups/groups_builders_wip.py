...
class GroupsBuilderSteps:
  ...
  def do_index_parts_tree(self):
    ...
    for part in parts:
      ...
      for edge in out_edges:
        ...
        parent_part_node = self.index.parts.setdefault(parent_part_number, Part(part_number=parent_part_number))
    ...
    for part_node in self.index.parts.values():
      if len(part_node.parents) == 0:
        if len(part_node.children) > 0:
          ...
          self.index.parts_roots_no_children_by_type.setdefault(part_node.part_type, {})[
  ...
  def do_index_property_use(self):
    ...
    for term_node in loinc_term_nodes:
      ...
      for edge in edges:
        ...
        if not self.primary and not isinstance(edge_type, LoincTermSupplementaryEdges):
          continue
        ...
        part_node.property_by_name_key.setdefault(simple_property_name, {})[related_key] = _property

        self.index.property_by_name_key.setdefault(simple_property_name, {})[related_key] = _property
      ...
      for relating_key, relating_property in term_properties.items():
        for related_key, related_property in term_properties.items():
          if related_key == relating_key:
            continue
          relating_property.related_properties_by_key[related_key] = related_property
          relating_property.related_properties_by_name_key.setdefault(related_property.get_simple_property_name(), {})[
  ...
  def _do_property_depths(self, _property: Property, depth: int, path: t.List[Property]):
  ...
  def _do_property_depths_percentage(self, _property: Property, path: t.List[Property]):
    if _property in path:
      return
    path.append(_property)
  ...
  def _infer_parent_properties(self, _property: Property, inferred_props: t.Dict[str, Property]) -> None:
    ...
    for parent_part in _property.part_node.parents.values():
      parent_property = Property(part_number=parent_part.part_number, part_name=parent_part.part_display,
